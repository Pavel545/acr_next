...
// Базовые CORS заголовки для публичного доступа
const CORS_HEADERS = {
...
function createDynamicEmailHTML(data: Record<string, any>, source: string): string {
  ...
  const subject = data._subject || 'Новая заявка';
  
  // Исключаем системные поля из вывода в самом письме
  const excludeFields = ['_subject', '_gotcha', 'privacy_policy', '_recipients'];
        ...
        <div style="width:600px; background: #eee; padding:0px; display: inline-block;">
          <div style="text-align: center; background-color: #fff; padding: 30px; border-radius: 3px; line-height: 24px;">
            <h2 style="margin-top: 10px; color:#000">
              Новая заявка с ресурса <span style="color: #203c97;">${escapeHtml(source)}</span>
            </h2>
...
export async function POST(request: NextRequest) {
  console.log('📨 Получен POST запрос на отправку почты');
  
  // 1. Проверка API-ключа
  const authHeader = request.headers.get('authorization');
  const apiKey = authHeader?.startsWith('Bearer ') ? authHeader.substring(7) : null;
  const validApiKey = process.env.API_MAIL_KEY; // Создайте эту переменную в .env

  if (!validApiKey || apiKey !== validApiKey) {
    console.log('❌ Ошибка авторизации: Неверный или отсутствующий API-ключ');
  ...
  try {
    ...
    if (body._gotcha && body._gotcha !== '') {
      ...
      return NextResponse.json({ success: true, message: 'OK' }, { headers: CORS_HEADERS });
    }
    
    // Проверка обязательных полей для связи
    ...
    // 3. Определение списка получателей письма
    // По умолчанию, если в body ничего не передали
    let recipients: string[] = ['acr-agency@yandex.ru', 'ForAnalyticss@yandex.ru'];
    
    if (body._recipients) {
      if (Array.isArray(body._recipients)) {
        recipients = body._recipients;
      } else if (typeof body._recipients === 'string') {
        // Если передали строкой через запятую: "mail1@ya.ru, mail2@ya.ru"
        recipients = body._recipients.split(',').map(email => email.trim()).filter(Boolean);
      }
    }

    if (recipients.length === 0) {
      return NextResponse.json(
        { error: 'Не указаны адресаты для отправки письма' },
        { status: 400, headers: CORS_HEADERS }
      );
    }
    
    const origin = request.headers.get('origin') || request.headers.get('referer') || 'Универсальный скрипт';
    const subject = body._subject || `Заявка от ${new Date().toLocaleString('ru-RU')}`;
    const html = createDynamicEmailHTML(body, origin);
    ...
    return NextResponse.json(
      { error: 'Ошибка отправки. Пожалуйста, попробуйте позже.', details: error.message },
